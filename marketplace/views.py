    ...
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            ...
            try:
                ...
                return JsonResponse({
                    'status': 'Success',
                    'message': 'Successfully added to the cart' if created else 'Increased the cart quantity',
    ...
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            ...
            try:
                ...
                return JsonResponse({'status': 'Failed', 'message': "Item doesn't exist in the cart", 'cart_counter': get_cart_count(request)})
        else:
            return JsonResponse({'status': 'Failed', 'message': 'Invalid request'})
    else:
        return JsonResponse({'status': 'login_required', 'message': 'Please login to continue'})
    ...
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            try:
                ...
                return JsonResponse({'status': 'Failed', 'message': "Item doesn't exist"})
            except Exception as e:
                print(e)
                return JsonResponse({'status': 'Failed', 'message': 'An error occured while removing the item'})
        else:
            return JsonResponse({'status': 'Failed', 'message': 'Invalid Request'})
    else:
